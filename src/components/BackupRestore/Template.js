...
'mongorestore{{#if host}} --host {{host}}{{/if}}{{#if port}} --port {{port}}{{/if}}{{#if database}} --db {{database}}{{/if}}{{#if username}} -u {{username}}{{/if}}{{#if password}} -p ******{{/if}}{{#if ssl}} --ssl{{/if}}{{#if authDb}} --authenticationDatabase {{authDb}}{{/if}}{{#if objcheck}} --objcheck{{/if}}{{#if oplogReplay}} --oplogReplay{{/if}}{{#if oplogLimit}} --oplogLimit {{oplogLimit}}{{/if}}{{#if restoreDbUsersAndRoles}} --restoreDbUsersAndRoles{{/if}}{{#if gzip}} --gzip{{/if}}{{#if drop}} --drop{{/if}}{{#if dryRun}} -q {{dryRun}}{{/if}}{{#if writeConcern}} --writeConcern{{writeConcern}}{{/if}}{{#if noIndexRestore}} --noIndexRestore{{/if}}{{#if noOptionsRestore}} --noOptionsRestore{{/if}}{{#if keepIndexVersion}} --keepIndexVersion{{/if}}{{#if maintainInsertionOrder}} --maintainInsertionOrder{{/if}}{{#if numParallelCollections}} --numParallelCollections {{numParallelCollections}}{{/if}}{{#if numInsertionWorkersPerCollection}} --numInsertionWorkersPerCollection {{numInsertionWorkersPerCollection}}{{/if}}{{#if stopOnError}} --stopOnError{{/if}}{{#if bypassDocumentValidation}} --bypassDocumentValidation{{/if}}{{#if output}} -o {{output}}{{/if}} \n';
