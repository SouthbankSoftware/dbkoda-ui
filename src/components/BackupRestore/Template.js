...
export const exportDB =
  '{{#each cols}}\n' +
  'mongoexport{{#if host}} --host "{{{escapeDoubleQuotes host}}}"{{/if}}{{#if port}} --port "{{{escapeDoubleQuotes port}}}"{{/if}} --db "{{{escapeDoubleQuotes database}}}"{{#if username}} -u "{{{escapeDoubleQuotes username}}}"{{/if}}{{#if password}} -p ******{{/if}}{{#if ssl}} --ssl{{/if}}{{#if collection}} --collection "{{{escapeDoubleQuotes collection}}}"{{/if}}{{#if authDb}} --authenticationDatabase "{{{escapeDoubleQuotes authDb}}}"{{/if}}{{#if pretty}} --pretty{{/if}}{{#if jsonArray}} --jsonArray{{/if}}{{#if noHeaderLine}} --noHeaderLine{{/if}} --type "{{{escapeDoubleQuotes exportType}}}"{{#if outputFields}} --fields "{{{outputFields}}}"{{/if}}{{#if query}} -q "{{{escapeDoubleQuotes query}}}"{{/if}}{{#if readPreference}} --readPreference "{{{escapeDoubleQuotes readPreference}}}"{{/if}}{{#if forceTableScan}} --forceTableScan{{/if}}{{#if skip}} --skip "{{{escapeDoubleQuotes skip}}}"{{/if}}{{#if limit}} --limit "{{{escapeDoubleQuotes limit}}}"{{/if}}{{#if exportSort}} --sort "{{{escapeDoubleQuotes exportSort}}}"{{/if}}{{#if assertExists}} --assertExists{{/if}}{{#if output}} -o "{{{escapeDoubleQuotes output}}}"{{/if}} ' +
  enter +
  '{{/each}}';

export const dumpDB =
  '{{#each cols}}\n' +
  'mongodump{{#if host}} --host "{{{escapeDoubleQuotes host}}}"{{/if}}{{#if port}} --port "{{{escapeDoubleQuotes port}}}"{{/if}} --db "{{{escapeDoubleQuotes database}}}"{{#if username}} -u "{{{escapeDoubleQuotes username}}}"{{/if}}{{#if password}} -p ******{{/if}}{{#if ssl}} --ssl{{/if}}{{#if collection}} --collection "{{{escapeDoubleQuotes collection}}}"{{/if}}{{#if authDb}} --authenticationDatabase "{{{escapeDoubleQuotes authDb}}}"{{/if}}{{#if gzip}} --gzip{{/if}}{{#if repair}} --repair{{/if}}{{#if oplog}} --oplog{{/if}}{{#if dumpDbUsersAndRoles}} --dumpDbUsersAndRoles{{/if}}{{#if viewsAsCollections}} --viewsAsCollections{{/if}}{{#if numParallelCollections}} --numParallelCollections "{{{escapeDoubleQuotes numParallelCollections}}}"{{/if}}{{#if query}} -q "{{{escapeDoubleQuotes query}}}"{{/if}}{{#if readPreference}} --readPreference "{{{escapeDoubleQuotes readPreference}}}"{{/if}}{{#if forceTableScan}} --forceTableScan{{/if}}{{#if output}}{{#if archive}} --archive={{else}} -o {{/if}}{{#if archive}}{{else}}"{{/if}}{{{escapeDoubleQuotes output}}}{{#if archive}}{{#if gzip}}.gz{{else}}.archive{{/if}}{{/if}}{{#if archive}}{{else}}"{{/if}}{{/if}} ' +
  enter +
  '{{/each}}';

export const dumpServer =
  '{{#each cols}}\n' +
