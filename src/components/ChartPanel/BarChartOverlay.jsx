 ...
 * @Last modified time: 2017-10-04T16:36:59+11:00
 *
...
class BarChartDropTarget extends React.PureComponent<*> {
  render() {
    const { target, grid, connectDropTarget, isOver } = this.props;
    const {
      chartWidth: width,
      chartHeight: height,
      height: gridHeight,
    ...
    if (target === 'y') {
      return connectDropTarget(
        <g className={classnames('YAxis', { over: isOver })}>
      ...
      return connectDropTarget(
        <g className={classnames('XAxis', { over: isOver })}>
          <path
      ...
      return connectDropTarget(
        <g className={classnames('Center', { over: isOver })}>
...
export default class BarChartOverlay extends React.PureComponent<*> {
  render() {
    ...
    let allowedOperations = getAllowedChartComponentOperations(valueSchemaPath, valueType);
    allowedOperations = _.filter(allowedOperations, v => v.action === 'load');
    ...
    return (
      <div className="BarChartOverlay">
        <svg width={width} height={height}>
          {_.map(allowedOperations, ({ target }) => (
            <BarChartDropTarget key={target} target={target} grid={grid} />
          ))}
        </svg>
      </div>
    );
