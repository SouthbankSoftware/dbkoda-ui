...
export default class TreePanel extends React.Component {
    ...
    this.reactionToTopology = reaction(() => this.props.store.topology.isChanged, () => {
      if (this.props.store.topology.isChanged && this.props.store.topology.json !== null) {
        this
          .props
          .treeState
          .parseJson(this.props.store.topology.json);
  ...
  render() {
    ...
    return (
      <div style={divStyle}>
        <TreeToolbar /> {this.treeStatus == 'NOPROFILE' && <div className="tree-msg-div">
          <span>Please reconnect profile to view its topology.</span>
        </div>}
        {this.treeStatus == 'LOADING' && <div className="tree-msg-div">
          <span>Loading topology...</span>
        </div>}
        {this.treeStatus == 'FAILED' && <div className="tree-msg-div">
          <span style={{
            color: 'red'
          }}>Failed to load topology.</span>
        </div>}
