...
export default class RadialWidget extends React.Component<Object, Object> {
  ...
  projection = () => {
    ...
    const key = items[0];
    if (key === 'network' || key === 'disk') {
      const downloadText = key === 'network' ? 'Download' : 'In';
      const uploadText = key === 'network' ? 'Upload' : 'Out';
      return {
        [downloadText]: (value: Object) => {
          if (value.value[key]) {
            return value.value[key].downloadDelta;
          }
        },
        [uploadText]: (value: Object) => {
          if (value.value[key]) {
            return value.value[key].uploadDelta;
          }
        }
      };
    }
